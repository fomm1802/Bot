    ...
    async def on_message(self, message):
        ...
        try:
            ...
            if urls:
                ...
                sender_avatar = sender.display_avatar.url  # ดึงรูปโปรไฟล์ (ป้องกัน NoneType)
                ...
                # เพิ่มลิงก์ลงใน Embed (แสดงแต่ละลิงก์เป็น field แยกกัน)
                for url in urls:
                    embed.add_field(name="🌐 ลิงก์ที่พบ", value=url, inline=False)

                # เพิ่มข้อมูลท้าย Embed (ชื่อผู้ส่งและไอคอน)
                ...
                if message.channel.permissions_for(message.guild.me).manage_messages:
                    ...
                    try:
                        ...
                        print("⚠️ ข้อความถูกลบไปแล้ว หรือไม่พบข้อความ")  # แจ้งเตือนถ้าข้อความหายไปก่อน
                    except discord.Forbidden:
                        print(f"⚠️ ไม่มีสิทธิ์ลบข้อความในช่อง {message.channel}")  # แจ้งเตือนถ้าบอทไม่มีสิทธิ์ลบ
                    except discord.HTTPException as e:
                        print(f"❌ ลบข้อความไม่สำเร็จ: {e}")  # แจ้งเตือนถ้าลบไม่สำเร็จ

        except Exception as e:
            print(f"❌ เกิดข้อผิดพลาดใน `on_message`: {e}")  # แจ้งเตือนข้อผิดพลาดในฟังก์ชัน
